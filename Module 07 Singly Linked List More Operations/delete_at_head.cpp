...
void delete_at_head(Node* &head) {
    Node* deleteNode = head; // Start from the head node
...
int main() {
    ...
    delete_at_head(head); // Delete the head node, will delete the first node 10
    delete_at_head(head); // Delete the new head node, will delete the second node 20
